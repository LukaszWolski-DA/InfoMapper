...
export function ObjectViewV2({
  ...
  // Filtered attributes from IntelligentFilter
  const [filteredAttributes, setFilteredAttributes] = useState<LogicalAttribute[]>(attributes)
  
  // Filter entities: only show entities that have at least one filtered attribute
  const filteredEntityIds = new Set(filteredAttributes.map(a => a.entityId))
  const filteredEntities = filteredAttributes.length === attributes.length
    ? entities // No filter applied, show all
    : entities.filter(e => filteredEntityIds.has(e.id)) // Only entities with matching attributes
        ...
        <div className="px-4 py-3 border-b border-gray-200 bg-white shrink-0">
          <div className="mb-3">
            ...
            <IntelligentFilter
              ...
              placeholder='Search attributes or use advanced: Concept = "Sales" AND IsPrimaryKey = "true"'
              showExamples={true}
            />
          </div>
